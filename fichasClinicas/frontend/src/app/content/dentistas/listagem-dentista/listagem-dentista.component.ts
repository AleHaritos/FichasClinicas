...
@Component({
  selector: 'app-listagem-dentista',
  templateUrl: './listagem-dentista.component.html',
  styleUrls: ['./listagem-dentista.component.css']
})
export class ListagemDentistaComponent implements OnInit {
    ...
    consultarPorData(): void {
      if(this.dia !== undefined) {
       ...
       this.dia = this.dia.toString() //Converter Date para string
        ...
        this.dentistaService.getPacienteByDate({diaConsulta: this.dia})
          .subscribe((res: any) => {
            this.dataSource = new MatTableDataSource(res)
            setTimeout(() => this.dataSource.paginator = this.paginator)
